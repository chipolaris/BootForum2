...
public class DynamicCriteriaQueryBuilder<T> {
    ...
    public DynamicCriteriaQueryBuilder(EntityManager entityManager, Class<T> entityClass, List<DynamicFilter> filters) {
    ...
    public List<T> getResultList() {
        ...
        for (DynamicFilter filter : filters) {
            try {
                predicates.add(buildPredicate(filter));
    ...
    public boolean exists() {
        ...
        List<Predicate> predicates = filters.stream()
                .map(filter -> buildPredicate(filter))
    ...
    public List<Predicate> buildPredicates() {
        ...
        for (DynamicCriteriaQueryBuilder.DynamicFilter filter : filters) {
            try {
                predicates.add(buildPredicate(filter));
            } catch (IllegalArgumentException e) {
                log.warn("Skipping invalid filter: {} due to {}", filter, e.getMessage());
    ...
    private Predicate buildPredicate(DynamicFilter filter) {
